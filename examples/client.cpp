...
int main()
{
    std::shared_ptr<asioudp> asioudp_ptr = std::make_shared<asioudp>(37001);
    while(1)
    {
        uint8_t data_buffer[] = {0x01, 0x02, 0x03, 0x04, 0x05};
        asioudp_ptr->send(asioudp::NORMAL_SEND, data_buffer, sizeof(data_buffer), "127.0.0.1", 36001);
        sleep(1);
    }
    asioudp_ptr->run();
